...
Mina kavatsesin algselt aga kasutada otse GPX andmete lisamiseks `leaflet` kaartile `leaflet.extras` paketi funktsiooni `addGPX()`, mis võtab andmed kas __a)__ gpx failist või __b)__ gpx xml stringist (pikk xml formaadis lause).
Niimoodi oleksin ma saanud kasutada algseid andmeid ja vältinud andmete vahepealset konverteeriminst `SpatialLines`, `SpatialPointsDataFrame` või muusse sellisesse objekti.


Oleks on paha poiss.
Tegelikult ma lõpuks vältisin oma andmete spatial klassi eksportimist, kuid ei kasutanud ka `addGPX()` formaati, sest mul oli vaja päevateekonnad mitmest trackist kokku panna ja selgus, et macis ei ole "Gpsbabel" programmi, mis seda teeb, võimalik niisama lihtsalt R-i kaudu käivitada kui näiteks Linuxis.
Seetõttu loobusin üldse gpx formaaadist ja liitsin andmed lihtsate andmeraamidena ja **eksportisin oma andmed geojson formaati**, kus sain neid leafletis ilusti kasutada.
